...
for "_i" from 1 to _amount do {
	...
	_xm = (BRPVP_mapaDimensoes select 0)/2;
	_ym = (BRPVP_mapaDimensoes select 1)/2;
	_angle = (_i - 1) * (360/_amount);
	_start = [[_xm,_ym,350],_xm min _ym,_angle] call BIS_fnc_relPos;
	_end = [[_xm,_ym,350],_xm min _ym,_angle] call BIS_fnc_relPos;
