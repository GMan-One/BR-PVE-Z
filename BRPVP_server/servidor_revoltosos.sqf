...
BRPVP_serverTrabalhando = ["SPAWNING REVOLTERS!",BRPVP_ant + "<t size='2'>REVOLTERS:</tr><br/><t size='3'>0</t>"];
...
for "_i" from 1 to _revQT do {
	...
	while {isNull _house} do {
		...
		if !(_try getVariable ["mapa",false]) then {_house = _try;};
	};
	_grupo = createGroup BLUFOR;
	_revoltoso = _grupo createUnit ["I_C_Soldier_Bandit_4_F",[0,0,0],[],0,"CAN_COLLIDE"];
	...
	_revoltoso setSkill 0.5;
	_angulo = random 360;
	_revoltoso setDir _angulo;
	BRPVP_revoltosos pushBack _revoltoso;
	_contaRev = _contaRev + 1;
	if (_contaRev mod 10 == 0) then {
		BRPVP_serverTrabalhando = ["",BRPVP_ant + "<t size='2'>REVOLTERS:</tr><br/><t size='3'>" + str _contaRev + "</t>"];
		PublicVariable "BRPVP_serverTrabalhando";		
	};
};
BRPVP_serverTrabalhando = ["",BRPVP_ant + "<t size='2'>REVOLTERS:</tr><br/><t size='3'>" + str _contaRev + "</t>"];
